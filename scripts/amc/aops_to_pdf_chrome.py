...
class AopsPdf(object):
    ...
    def remove_nodes(self, container):
        ...
        for elem in container.query_selector_all('h2 span'):
            if 'See Also' in elem.text_content():
    ...
    def save_solutions(self):
        ...
        for i in range(1, 3):
            ...
            container = self.page.query_selector('div.mw-content-ltr')
            self.remove_nodes(container)
            [elem.evaluate('x => x.remove()') for elem in self.page.query_selector_all('h1.firstHeading')]
            [elem.evaluate('x => x.remove()') for elem in self.page.query_selector_all('div#contentSub')]
            # 修改这行代码
            self.page.query_selector('h2 span#Problem').evaluate('(x, text) => x.textContent = text', f'Problem {i}')

            name = f'Problem_{i}_Solution'
            self.save_pdf(name)
